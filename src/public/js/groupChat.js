...
function callCreateGroupChat() {
  $("#btn-create-group-chat").unbind("click").on("click", function () {
    ...
    if(countUsers.length < 2) {
      alertify.notify("Vui lòng chọn thêm bạn bè, một cuộc trò chuyện nhóm phải có 3 người", "error", 7);
      return false;
    }
...
$(document).ready(function () {
  ...
  socket.on("response-new-group-created", function(response) {
    ...
    let rightSideData = `
     ...
     <div class="top">
             ...
             <span class="chat-menu-right">
                 <a href="javascript: void(0)" class="number-messages" data-toggle="modal">
                     <span class="show-number-messages">
                     ${response.groupChat.messageAmount}
                     </span>
                     <i class="fa fa-comment-o"></i>
                 </a>
             </span>
    ...
    //Srep 9: update when member received a group chat
    socket.emit("member-recived-group-chat", {groupChatId: response.groupChat._id});
